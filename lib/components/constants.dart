// Constant Color and Value used
import 'package:flutter/material.dart';

const kBottomContainerHeight = 70.0;

const kActiveColor = Color(0xFF1D1E33);

const kInactiveColor = Color(0xFF111328);
const kFloatingIconColor = Color(0xFF4C4F5E);

const kBottomColor = Color(0xFFEB1555);
const kIconsSize = TextStyle(fontSize: 30.0, fontWeight: FontWeight.w900);

const kTextStyle = TextStyle(
  fontSize: 18.0,
  color: Color(0xFF8D8E98),
);
const kTitleStyle = TextStyle(fontWeight: FontWeight.bold, fontSize: 40);
const kResultTextStyle = TextStyle(
    fontSize: 22.0, color: Color(0xFF24D876), fontWeight: FontWeight.bold);
const kBMITextStyle = TextStyle(fontSize: 100.0, fontWeight: FontWeight.bold);
const kBMIResultStyle = TextStyle(fontSize: 22.0);
// Int Values
int kHeightValue = 180;
int kWeightValue = 40;
int kAgeValue = 18;
